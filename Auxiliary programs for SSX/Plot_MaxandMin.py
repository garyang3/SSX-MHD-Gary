'''My own version of plotting tools to plot maximum/minumum values from simulation out of hdf5 files'''
import sys
import pathlib
import h5py
import os
...
axs[0, 0].set_ylim([2, 8.1])
axs[0, 0].set_title("max_v")
axs[1, 0].plot(sim_time, max_J,marker='.')
axs[1, 0].set_ylim([25, 260])
axs[1, 0].set_title("max J")
axs[1, 0].sharex(axs[0, 0])
axs[0, 1].plot(sim_time, max_B,marker='.')
axs[0, 1].set_ylim([2.5, 4.5])
axs[0, 1].set_title("max_B")
axs[1, 1].plot(sim_time, resistivity,marker='.')
axs[1, 1].set_ylim([3000, 4000])
axs[1, 1].set_title("resistivity")
fig.tight_layout()
plt.savefig('max v J B Resistivity.png')
